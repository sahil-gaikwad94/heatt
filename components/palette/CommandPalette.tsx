...
export function CommandPalette() {
  ...
  const cmds = React.useMemo<Cmd[]>(() => {
    ...
    const act: Cmd[] = [
      ...
      { id: 'a-refresh', group: 'Toggle', label: 'Re-fetch the syndicated wire', icon: '↻', run: () => { app.refresh(true); app.toast('Refreshing the wire…', 'cool'); } },
    ...
    const heatTargets = [...app.posts]
      ...
      .map<Cmd>((p) => ({
        id: `p-${p.id}`,
        group: 'Heat-ranked content',
        label: (p.title ?? p.text ?? '').slice(0, 76),
        hint: `@${p.authorHandle} · ${timeAgo(p.date)} · ${kelvin(p.heat?.temp ?? 0)}`,
        icon: p.kind === 'forge' ? '⚒' : '✦',
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-start justify-center px-3 pt-[8vh]" role="dialog" aria-modal>
      <div className="absolute inset-0 bg-[radial-gradient(80%_60%_at_50%_20%,rgba(255,92,10,.1),transparent_60%)] backdrop-blur-md" style={{ background: 'rgba(4,4,6,.72)' }} onClick={() => app.setPalette(false)} />
      <motion.div
        initial={{ opacity: 0, y: -14, scale: 0.985 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
        className="ht-glass relative w-full max-w-[620px] overflow-hidden rounded-[22px]"
        style={{ boxShadow: '0 50px 130px -40px rgba(0,0,0,1), 0 0 0 1px rgba(255,138,31,.12)' }}
      >
                ...
                return (
                  <button
                    ...
                    className={cls(
                      ...
                      idx === i ? 'bg-[linear-gradient(90deg,rgba(255,45,18,.18),rgba(255,181,49,.05))] text-ink' : 'text-ink-dim hover:bg-white/[.04]'
                    )}
                  >
        ...
        <div className="flex items-center justify-between border-t border-white/[.07] px-4 py-2.5 text-[11px] text-ink-faint">
          <span className="flex items-center gap-2">
            ...
            @<b>{s.me?.handle ?? 'you'}</b> · mass {(s.me?.thermalMass ?? 1).toFixed(2)}
          </span>
          <span className="flex items-center gap-2">
