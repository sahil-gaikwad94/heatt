...
export default function App() {
  ...
  const surfaceAtmosphere: ThemeId = page === 'home' || page === 'journal' ? 'midnight' : page === 'create' ? 'ember' : 'paper'
...
function LandingPage({ onEnter, onExplore, onSignIn }: { onEnter: () => void; onExplore: () => void; onSignIn: () => void }) {
  ...
  return <div className="landing-page">
    ...
    <main>
      ...
      <article className="landing-real-content"><div className="landing-source-head"><span className={`source-mark source-${featuredSource.accent}`}>{featuredSource.name.split(/\s+/).map(word => word[0]).join('').slice(0, 2)}</span><div><strong>{featuredSource.name}</strong><span>Original publisher</span></div></div><blockquote>“{featuredWisdom.sourceText}”</blockquote><p>{featuredWisdom.interpretation}</p><footer><span>{featuredWisdom.source}</span><a href={featuredSource.url} target="_blank" rel="noopener noreferrer">Visit {featuredSource.name}</a></footer></article></section>
